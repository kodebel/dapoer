/*
  ...
  - The values [DIREKTUR,DIREKTUR_UTAMA] on the enum `Pengguna_posisi` will be removed. If these variants are still used in the database, this will fail.
...
-- AlterTable
ALTER TABLE `pengguna` MODIFY `posisi` ENUM('STAFF', 'LEADER', 'ASMAN', 'MANAGER', 'Dir_Operasional', 'KOMISARIS', 'CEO') NOT NULL;
...
-- AddForeignKey
ALTER TABLE `image` ADD CONSTRAINT `Image_userId_fkey` FOREIGN KEY (`userId`) REFERENCES `pengguna`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
